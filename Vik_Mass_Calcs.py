...
# ====================================================================================
# Load gas profile data for TNG100 and TNG300 simulations from HDF5 files
...
# Define the critical density of the universe (in units of kpc^-3)
rho_crit = constants.RHO_CRIT_0_KPC3

# CLUSTER DICTIONARY
# ====================================================================================
# Define a dictionary of clusters with their names, redshift (z), and R500 (in kpc)
# R500 represents the radius within which the mean density is 500 times the critical density.
cluster_dict = {
...
# List of selected clusters for analysis
clusters = ['A2390', 'A133']

# Define a custom cosmology with parameters based on the Planck 2018 cosmology
...
# Define constants
gamma = 3  # Slope parameter for density profiles
mp = constants.M_PROTON  # Mass of a proton
...
def get_rho_norm(r, n0, rc, rs, a, B, epsilon, n02, rc2, B2):
    # Compute electron and proton density based on model parameters
    npne = (
        ((n0 * (10**-3))**2)
    ...
    # Calculate gas density (rho_g)
    rho_g = 1.624 * mp * (npne)**(1/2)
    
    # Calculate cosmological parameters for normalization
    H_col = Vik_Cosmo.Hz(z)  # Hubble parameter at redshift z
    h_col = H_col * (10 ** -2)  # Hubble parameter in reduced units
...
# Define a similar function for a normalized fit
def get_rho_norm_fit(r, n0, rc, rs, a, B, epsilon, n02, rc2, B2):
    # Similar to get_rho_norm but only returns normalized density
    npne = (
        ((n0 * (10**-3))**2)
        * (((r / rc)**(-a)) / ((1 + (r**2) / (rc**2))**(3 * B + (-a / 2))))
        * (1 / ((1 + ((r**gamma) / (rs**gamma)))**(epsilon / gamma)))
...
# Extract data from gas profiles in TNG100 and normalize
radii_all100 = np.array(gasprofs100_v5_vals['profile_bins'])  # Radii bins
R_Crit500_100 = np.array(gasprofs100_v5_vals['catgrp_Group_R_Crit500'])  # R500 values
radii_all100_norm = radii_all100 / R_Crit500_100[:, None]  # Normalize radii
rho_vals100 = np.array(gasprofs100_v5_vals['profile_gas_rho_3d'])  # Gas density
rho_vals100_norm = rho_vals100 / rho_crit  # Normalize gas density

# Extract data from gas profiles in TNG300 and normalize
...
# Normalize radii for each cluster by their R500 values
# This normalization helps in comparing density profiles across different clusters
...
# MASS CALCULATIONS
# ====================================================================================
# Define mass values from the Vikhlinin model for different clusters
# Format: [total mass (10^14 Msun), gas fraction], where some entries are missing (None)
Vik_mass_vals = [
    [3.17, 0.083], [None, None], [3.06, 0.124], 
...
# Convert total mass from 10^14 Msun to Msun
Vik_mass_vals[:, 0] *= 10**14

# Calculate the gas mass for each cluster: gas mass = total mass * gas fraction
Vik_gas_mass = Vik_mass_vals[:, 0] * Vik_mass_vals[:, 1]

# Add the gas mass as a third column to the mass values array
Vik_mass_vals = np.column_stack((Vik_mass_vals, Vik_gas_mass))

# Define the integrand function to calculate gas density profiles
def integrand(r, n0, rc, rs, a, B, epsilon, n02, rc2, B2):
    # Get the gas density profile from the get_rho_norm function
    rho_g, rho_norm = get_rho_norm(r, n0, rc, rs, a, B, epsilon, n02, rc2, B2)
    # Return the integrand: rho_g * 4πr^2
    return rho_g * 4 * np.pi * (r**2) 

# Perform integration to calculate the total gas mass within R500 for each cluster
# The integration is carried out using the scipy quad function over the range [0, R500]
...
# Calculate median gas mass from TNG100 and TNG300 simulations
# These calculations are based on volume and density profiles within R500

# Extract volume profiles and normalize radii for TNG100 and TNG300
...
# Determine indices where radii exceed R500
...
# Calculate gas mass for each cluster in TNG100
...
# Calculate gas mass for each cluster in TNG300
...
# Compute total cluster mass statistics for TNG300 and TNG100
...
# PLOTTING
# ====================================================================================
# Plotting cluster mass statistics and gas mass vs. total mass from simulations and models

# Plot 1: Bar chart of median cluster masses and gas masses
# ====================================================================================
# Uncomment the following block to visualize a bar chart of median mass values
...
# 
# plt.bar(clusters, values, color='maroon', width=0.4)
# ax.set_yscale('log')  # Use logarithmic scale for y-axis due to large range of mass values
# 
# plt.xlabel("Clusters")
...
# Scatter plot for TNG100 and TNG300 cluster gas masses against total masses
...
# Highlight the first data points to add labels for TNG100 and TNG300
...
# Add scatter points for individual clusters based on Vikhlinin model values
# Each point is labeled with its cluster name and styled with distinct colors
...
plt.xlabel('M$_{total,500c}$', fontsize=20)  # Total mass within R500
plt.ylabel('M$_{gas,500c}$', fontsize=18)  # Gas mass within R500
